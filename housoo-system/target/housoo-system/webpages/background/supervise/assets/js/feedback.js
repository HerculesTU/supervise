    ...
    if (content) {
        parent.layer.confirm(msg ? msg : "确认提交？", {
            ...
            PlatUtil.ajaxProgress({
                url: "/supervise/TakerController.do?takerHandleFeedback",//承办人提交反馈
                params: {
                    SUPERVISE_ID: supervise.RECORD_ID,
                    FEEDBACK_CONTENT: $(".feedback").find("textarea[name='fknr']").val(),
                    REMARKS: $(".feedback").find("input[name='bz']").val(),
                    FILE_URL: $(".feedback").find("#fileurl").val(),
                    FILE_NAME: $(".feedback").find("input[name='scfj']").val(),
                    NODE_ID: nodeId,
                    STATUS: ($(".feedback").find("input[name='fk']").val().indexOf('已逾期') != '-1' && msg.indexOf('重新') == '-1') ? 2 : 1
                },
 ...
 * 督办确认
 */
function confirmSupervise(msg, IS_ACCEPT) {
    var remarks = $(".feedback").find("textarea[name='bz']").val();
    if (IS_ACCEPT == '2' && !remarks) {
        layer.msg("请填写拒收说明！");
        return false;
    }
    parent.layer.confirm(msg, {
        ...
        PlatUtil.ajaxProgress({
            url: "/supervise/TakerController.do?takerConfirmSupervise",//承办人督办确认
            params: {
                SUPERVISE_ID: supervise.RECORD_ID,
                REMARKS: $(".feedback").find("textarea[name='bz']").val(),
                NODE_ID: nodeId,
                STATUS: ($(".feedback").find("input[name='fk']").val().indexOf('已逾期') != '-1') ? 2 : 1,
                IS_ACCEPT: IS_ACCEPT
            },
            callback: function (data) {
                if (data.success) {
...
function loadTakerFeedbackInfo(checkNodeId) {
    ...
    PlatUtil.ajaxProgress({
        url: "/supervise/TakerController.do?loadTakerFeedbackInfo",
        params: {
