...
<html>
  ...
  <body>
   ...
   <div class="panel-Title">
	<h5 id="tb_supervise_clazz_datatablepaneltitle">督办类型表单</h5>
</div>
<div class="titlePanel">
	<div class="title-search form-horizontal" id="tb_supervise_clazz_search" formcontrolid="402881f67171942a0171719c5e110015">
		<table>
			<tr>
				<td>查询条件</td>
					...
					<div class="table-filter">
						...
						<div class="table-filter-list" style="width: 420px; display: none;max-height: 280px;">
							...
							<div class="table-filter-list-bottom">
								<a onclick="PlatUtil.tableSearchReset('tb_supervise_clazz_search');" class="btn btn-default" href="javascript:void(0);"> 重 置</a> 
								<a onclick="PlatUtil.tableDoSearch('tb_supervise_clazz_datatable','tb_supervise_clazz_search',true);" class="btn btn-primary" href="javascript:void(0);"> 查 询</a>
							</div>
						</div>
					</div>
				</td>
...
<div class="gridPanel">
	<table id="tb_supervise_clazz_datatable"></table>
</div>
<div id="tb_supervise_clazz_datatable_pager"></div>
<script type="text/javascript">
function addJqGridRecordInfo(RECORD_ID){
	var url = "supervise/SuperviseClazzController.do?goForm&UI_DESIGNCODE=supervise_clazz_form";
	var title = "新增督办类型信息";
	if(RECORD_ID){
	...
	PlatUtil.openWindow({
	  ...
	  content: url,
		  ...
		  if(PlatUtil.isSubmitSuccess()){
              //弹出框提交成功后,需要回调的代码
			  $("#tb_supervise_clazz_datatable").trigger("reloadGrid"); 
		  }
	  }
	});
}
function editJqGridRecordInfo(){
	var rowData = PlatUtil.getTableOperSingleRecord("tb_supervise_clazz_datatable");
	if(rowData){
...
function delJqGridRecordInfo(){
	PlatUtil.operMulRecordForTable({
		tableId:"tb_supervise_clazz_datatable",
		selectColName:"RECORD_ID",
		url:"supervise/SuperviseClazzController.do?multiDel",
...
$(function(){
	PlatUtil.initJqGrid({
		  tableId:"tb_supervise_clazz_datatable",
		  searchPanelId:"tb_supervise_clazz_search",
		  url: "appmodel/CommonUIController.do?tabledata&FORMCONTROL_ID=402881f67171942a0171719c5e110015",
