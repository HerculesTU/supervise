...
<html>
  ...
  <body>
   ...
   <div class="ui-layout-north" platundragable="true" compcode="direct_layout">
      <plattag:select name="COMPANY_ID" allowblank="true" auth_type="write" value="${sessionScope.__BACKPLATUSER.SYSUSER_COMPANYID}" istree="true" onlyselectleaf="false" placeholder="请选择单位" comp_col_num="0" dyna_interface="commonUIService.findGenTreeSelectorDatas" dyna_param="[TABLE_NAME:PLAT_SYSTEM_COMPANY],[TREE_IDANDNAMECOL:COMPANY_ID,COMPANY_NAME],[TREE_QUERYFIELDS:COMPANY_PARENTID,COMPANY_PATH],[FILTERS:COMPANY_PARENTID_EQ|${sessionScope.__BACKPLATUSER.SYSUSER_COMPANYID}]">
   </plattag:select>
</div>
  ...
  $(function(){
	...
	$("select[name='COMPANY_ID']").change(function() {
		...
		if(COMPANY_ID){
	        ...
	        zTree.setting.async.otherParam["Q_DEPART_COMPANYID_EQ"] = COMPANY_ID;
	  ...
	  PlatUtil.initZtree({
		  ...
		  async : {
			url:"system/DepartController.do?departAndUsers",
			otherParam : {
				//树形表名称
				...
				"Q_DEPART_COMPANYID_EQ":backLoginUser.SYSUSER_COMPANYID,
				//是否显示树形标题
