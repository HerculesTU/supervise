	...
	<div class="tab-content">
		<div id="tab-1" class="tab-pane active">
			...
			<jsp:include page='<%="/appmodel/DesignController/includeUI.do"%>' >
			    <jsp:param value="tableColList" name="DESIGN_CODE"/>
			</jsp:include>
			<!-- 结束引入表格列配置 -->
		</div>
...
function saveEditTableAttachConfig(){
	var result = false;
	var tableColFields = $("#tableCol_datatable").jqGrid("getRowData");
	var TABLECOLIDS = "";
	if(tableColFields.length>0){
		$.each(tableColFields,function(index,obj){
			if(index>0){
				TABLECOLIDS+=",";
			}
			TABLECOLIDS+=obj.TABLECOL_ID;
		});
	}
