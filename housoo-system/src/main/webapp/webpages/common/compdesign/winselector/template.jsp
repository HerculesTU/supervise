...
>
   ...
   <plattag:winselector name="${CONTROL_NAME}" allowblank="${ALLOW_BLANK}" auth_type="${CONTROL_AUTH}" 
      value="${CONTROL_VALUE!''}" selectorurl="${SELECTORURL}" title="${WIN_TITLE}" 
      width="${WIN_WIDTH}" height="${WIN_HEIGHT}" selectedlabels="${SELECTEDLABELS!''}"
      <#if IS_TREESELECTOR=='true' >
         <#if CHECKCASCADEN?? >checkcascaden="${CHECKCASCADEN?replace(',','')}"</#if>
         <#if CHECKCASCADEY?? >checkcascadey="${CHECKCASCADEY?replace(',','')}"</#if>
         checktype="${CHECKTYPE}"
      </#if>
      <#if ALLOW_BLANK=='false' >datarule="required;" </#if>
      <#if MAXSELECT?? >maxselect="${MAXSELECT}"</#if>
      <#if MINSELECT?? >minselect="${MINSELECT}"</#if>
