...
<form method="post" class="form-horizontal" id="BaseConfigForm">
	...
	<div class="hr-line-dashed"></div>
	<div class="form-group">
		<plattag:input name="STEP" auth_type="write" allowblank="true"  datarule="digits;"
			placeholder="请输入输入跨度值" comp_col_num="3" label_col_num="1" value="${fieldInfo.STEP}"
			maxlength="30" label_value="跨度值"  >
		</plattag:input>
		<plattag:input name="DECIMALS" auth_type="write" allowblank="true"  datarule="positiveInteger;"
			placeholder="请输入输入小数位数" comp_col_num="3" label_col_num="1" value="${fieldInfo.DECIMALS}"
	...
	<div class="form-group">
	    <plattag:input name="PREFIX" auth_type="write" allowblank="true" attach_props=""
			placeholder="请输入前标签" comp_col_num="3" label_col_num="1" value="${fieldInfo.PREFIX}"
			maxlength="100" label_value="前标签" >
		</plattag:input>
		<plattag:input name="POSTFIX" auth_type="write" allowblank="true" 
			placeholder="请输入后标签 " comp_col_num="7" label_col_num="1" value="${fieldInfo.POSTFIX}"
	...
	$("input[name='AJAX_ABLE']").change(function() {
		...
		if(ajaxAble=="1"){
			...
			if(!AJAX_URL){
				...
				ajaxUrl+="&VALID_FIELDNAME="+CONTROL_NAME+"]";
