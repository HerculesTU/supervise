...
function bindEvent() {
    ...
    $(".icon-bianji").click(function () {
        ...
        var url = "/supervise/TakerController.do?goTakerSuperviseInfo&isRead=1&id=" + id;
        if (status == '9') {
...
function loadTakerSuperviseList(clazzId) {
    ...
    PlatUtil.ajaxProgress({
        url: "/supervise/TakerController.do?findTakerSuperviseList",
        progressMsg: "刷新中...",
        params: {
            status: status,
            clazzId: clazzId
...
function viewSupervise(id, nodeId, status) {
    ...
    var b = ['6', '8'];
    var isRead = '0';
    if ((nodeId == '2' && status != '3') || (nodeId == '3' && (a.indexOf(status) == -1)) || (nodeId == '5' && (b.indexOf(status) == -1))) {
        isRead = '1';//只读
    }
    var frameId = window.frameElement && window.frameElement.id || '';
    var url = "/supervise/TakerController.do?goTakerSuperviseInfo&isRead=" + isRead + "&id=" + id + "&nodeId=" + nodeId + "&parentFrameId=" + frameId;
    if (status == '9') {
        url = "/supervise/TakerController.do?goTakerSupEndInfo&id=" + id;
    }
    parent.PlatTab.newTab({
