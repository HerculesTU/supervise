...
/**
 * 描述：Word 文件中标签的封装类，保存了其定义和内部的操作
 *
 * @author zxl
 * @version 1.0
 * @time 2018-08-04
...
public class BookMark {
    ...
    private void insertAfterBookmark(XWPFRun run) {
        ...
        if (insertBeforeNode != null) {
            this.paragraph.getCTP().getDomNode().insertBefore(
    ...
    private void replaceBookmark(XWPFRun run) {
        ...
        if (!nodeStack.isEmpty()) {
            ...
            //this.paragraph.getCTP().getDomNode().removeChild(lastRunNode);
    ...
    private void deleteChildNodes(Stack<Node> nodeStack) {
        ...
        for (int i = 1; i < nodeStack.size(); i++) {
            ...
            if (toDelete.getNodeName().contains(BookMark.BOOKMARK_START_TAG)) {
                ...
                if (!inNestedBookmark) {
                    this.paragraph.getCTP().getDomNode().removeChild(toDelete);
                }
            }
        }
    }
